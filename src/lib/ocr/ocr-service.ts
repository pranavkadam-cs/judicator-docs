...
// Default singleton provider (swappable for testing or future providers)
let defaultProvider: OCRProvider = new TesseractProvider();
...
export async function processDocumentOCR(
  ...
  provider: OCRProvider = defaultProvider,
): Promise<OCRResult> {
  ...
  try {
    ...
    if (isPdfFile(mimeType, filename)) {
      ...
      if (pdfExtract.extractedImages && pdfExtract.extractedImages.length > 0) {
        ...
        for (let i = 0; i < pdfExtract.extractedImages.length; i++) {
          ...
          if (!imgBuf) continue;
          try {
            const pageOcr = await provider.recognize(imgBuf, lang);
    ...
    if (isImageFile(mimeType, filename)) {
      const ocrRes = await provider.recognize(input.fileBuffer, lang);
